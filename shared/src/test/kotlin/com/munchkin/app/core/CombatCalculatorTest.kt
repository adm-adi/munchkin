...
class CombatCalculatorTest {
    ...
    private fun createPlayer(
        ...
        level: Int,
        gear: Int,
        raceIds: List<EntryId> = emptyList(),
        classIds: List<EntryId> = emptyList()
    ): PlayerState {
        return PlayerState(
    ...
    private fun createCatalogEntry(entryId: EntryId, displayName: String): CatalogEntry {
        return CatalogEntry(
            entryId = entryId,
            displayName = displayName,
            normalizedName = CatalogEntry.normalize(displayName),
    ...
    fun `test warrior wins ties`() {
        ...
        val p1 = createPlayer("p1", 5, 0, classIds = listOf(warriorClassId))
        ...
        val gameState = GameState(
            ...
            classes = mapOf(warriorClassId to createCatalogEntry(warriorClassId, "Guerrero"))
        )
    ...
    @Test
    fun `test cleric bonus works with catalog class`() {
        val clericClassId = EntryId("custom_cleric")
        val player = createPlayer("p1", 5, 0, classIds = listOf(clericClassId))
        val undeadMonster = createMonster(7, 0).copy(isUndead = true)
        ...
        val gameState = GameState(
            ...
            players = mapOf(PlayerId("p1") to player),
            classes = mapOf(
                clericClassId to createCatalogEntry(clericClassId, "Clérigo")
            )
        )

        val combatState = CombatState(
            mainPlayerId = PlayerId("p1"),
            monsters = listOf(undeadMonster)
        )

        val result = CombatCalculator.calculateResult(combatState, gameState)

        assertEquals(8, result.heroesPower)
        assertEquals(7, result.monstersPower)
        assertEquals(CombatOutcome.WIN, result.outcome)
    }

    @Test
    fun `test elf helper bonus works with catalog race`() {
        val elfRaceId = EntryId("custom_elf")
        val mainPlayer = createPlayer("p1", 6, 0)
        val helper = createPlayer("p2", 2, 1, raceIds = listOf(elfRaceId))
        val monster = createMonster(9, 0)
        ...
        val gameState = GameState(
            ...
            hostId = PlayerId("p1"),
            players = mapOf(
                PlayerId("p1") to mainPlayer,
                PlayerId("p2") to helper
            ),
            races = mapOf(
                elfRaceId to createCatalogEntry(elfRaceId, "Elfo")
            )
        )
        ...
        assertEquals(9, result.heroesPower)
        assertEquals(9, result.monstersPower)
        assertEquals(CombatOutcome.LOSE, result.outcome)

        val winningResult = CombatCalculator.calculateResult(
            combatState.copy(tempBonuses = listOf(TempBonus("bonus", "Potion", 1, BonusTarget.HEROES))),
            gameState
        )
        assertEquals(1, winningResult.helperLevelsGained)
        assertEquals(CombatOutcome.WIN, winningResult.outcome)
    }
